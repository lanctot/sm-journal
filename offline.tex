...
\Repeat{$\alpha_s = \beta_s$}{
	\For{$r \in \cA'_i$, $c \in \cA'_{-i}$}{
		\If{$A'_{rc}$ is not initialized}{\label{alg:doab:restr1}
			...
			\If{$v^{-i}_{s'} < v^i_{s'}$} {
				$A'_{rc} \leftarrow A'_{rc} + \Delta_{c}(\cT(s,r,c),s')\cdot \textrm{double-oracle}(s',i,v^{-i}_{s'},v^i_{s'})$\label{alg:doab:recursive}\;
				}
			\Else{
...
%			$v^{-i}_{s'} \leftarrow \textrm{alpha-beta}(s',-i)$\; \label{alg:br:ab2}
			\If{$v^{-i}_{s'} < v^i_{s'}$} {
				$v_{a_i,a_{-i}} \leftarrow v_{a_i,a_{-i}} + \Delta_c(\cT(s,a_i,a_{-i}),s')\cdot$ $\textrm{double-oracle}(s',i,v^{-i}_{s'},v^i_{s'})$\label{alg:br:recursive}\;	
				}
			\Else{
