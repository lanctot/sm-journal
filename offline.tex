...
solves the matrix game $A$ using standard linear program (LP) for solving normal-form games\footnote{\reviewchange{By solving a game we mean computing both the optimal value and the strategy that achieves it.}}:
\begin{eqnarray}
\max & v_s & \\
...
Furthermore, we can generalize this argument using induction to game trees with multiple simultaneous moves: when solving a serialized game with $(d+1)$ steps, we know that solving each subgame with $d$ steps yields an upper bound on the value of that subgame from the perspective of player $i$. 
...
Firstly, recall that optimal strategies exist for simultaneous move games as a result of von Neumman's minimax theorem, which guarantees each player 
...
are inherently determined by the number of dominated actions in each subgame game, the specific serializations, and/or order used to expand the
...
When using RM or Exp3 and back-propagating the mean value of all samples at each state, then an eventual convergence can be guaranteed by
induction~\cite{lisy2013-nips};
however, back-propagation of the sampled value (as done on line~\ref{alg:smmcts:return} of Algorithm~\ref{alg:smmcts}) seems to work better in practice.
Also, the proof does not include a finite-time bound, so there is no known guarantee for the quality of the solution if Algorithm~\ref{alg:smmcts} is
terminated early.
Also, since these algorithms have a fixed exploration parameter $\gamma$, there is an easy-to-construct example game that if the strategy can converges 
to an $\epsilon$-NE it can be no closer than $\epsilon \ge \gamma D > 0$, where $D$ is the depth of the game~\cite{Kovarik2015Analysis}.
\mlanctot{@Vilo, please check that I interpreted this correctly.}
...
algorithm MCCFR. By~\cite[Theorem 5]{Lanctot09Sampling}, when OOS is run from the root, with probability $1-p$ an $\epsilon$-NE will is reached after
$O(\frac{(|\cA||\cS|^2}{p \delta^2 \epsilon^2})$ iterations, where $|\cA| = \max_{s \in \cS, i \in \{1,2 \}}{|\cA_i(s)|}$ and $\delta$ is the smallest
probability of sampling any single $z \in \cZ$. We will see in Section~\ref{sec:eval:offline} that this bound is often loose in practice. 
