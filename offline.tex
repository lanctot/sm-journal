...
We can see that the OS bound dominates the time required to build the tree. A single explorative action is taken with probability $\gamma/|\cA|$ and so the probability of sampling a terminal $z$ only due to exploration is $\frac{1}{\delta}=(\frac{|\cA|}{\gamma})^{2D}$, $D^2 < |\cA|^{2D} \in O(|\cS|)$ for any $\cA$, and we can set $p_1=p_2=p/2$. Then the probability that both the tree will be built and the convergence will be achieved can be bounded by $(1-p_1)(1-p_2) \geq (1-p)$. \qed
\end{proof}
