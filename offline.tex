...
%\jcermak{The only part mentioning strategy retrieval, should it be moved to online alg. description?} 
...
The main goal of the double-oracle algorithm is to find a solution of a matrix game without necessarily constructing the complete linear program that solves this game. 
...
$T$ represents the MCTS tree, in which each state is represented by one node.\bbosansky{I'd use $\cS'$ or something based on $\cS$ instead of $T$.}
