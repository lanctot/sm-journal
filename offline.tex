...
The baseline algorithm for solving simultaneous move games exactly is backward induction~(Section~\ref{sec:algs:bi}).
...
Then, we further improve the algorithm by speeding up the calculation of NE in stage games, exploiting the iterative framework known as the double-oracle algorithm~(Section~\ref{sec:algs:doab}).
...
\small
\SetKwInOut{Input}{input}\SetKwInOut{Output}{output}
\Input{$s$ -- current state of the game; $i$ -- current player; $\pi_i$ -- reach probability for player $i$; $\pi_{-i}$ -- reach probability for opponent of $i$, $p$ -- probability of smapling $s$}
