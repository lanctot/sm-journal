...
As for computational complexity, the time cost per node is linear in $|\cA_i|$ in UCT and RM. The time cost per node is quadratic in the case of Exp3 due to the numerically stable update rule (Equation~\ref{eq:exp3:stable}.) 
The memory required per node is linear in UCT and Exp3, and quadratic in RM due to storing estimates of each child subgame.
...
First, the whole game tree is eventually built by the algorithm.
 A direct consequence of Lemma 43 in \cite{Kovarik2015Analysis} is that the tree of depth $D$ is built with probability $(1-p_1)$ in less than $$16 D \left(\frac{|\cA|}{\gamma}\right)^{2D} \max(D,4 \log p_1^{-1} + 4)$$ iterations by an algorithm with a fixed exploration $\gamma$.
