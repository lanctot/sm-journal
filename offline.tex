...
The backward induction algorithm can be easily enhanced by using bounds on the value of subgames.
These bounds can be calculated efficiently by using a transformation of a simultaneous move game into a perfect information extensive-form 
game and applying wide range of search improvements developed for this more standard setting.
...
\reviewchange{
We call is purely alternating form a {\it serialization} of the original game.
Given this advantage, the value of a serialized game consisting of a single simultaneous move such that player $i$ is second to move
is greater or equal to the value of the original simultaneous move game from the perspective of player~$i$.
...
we know that solving each subgame with $d$ steps acheives an upper bound on the value of that subgame from the perspective of player $i$. 
...
By using bounds calculated in this way, we can speed-up the backward induction algorithm, using alpha-beta style bounds (denoted $\biab$).
...
We distinguish two cases when extracting the equilibrium strategies from the $\biab$. %backward induction with alpha-beta search.
...
In the worst case, the only NE requires mixed strategies in each state of the game.
...
First of all, we describe the main principles of the double-oracle algorithm for matrix games, followed by describing the application of the algorithm in simultaneous move game~\cite{Bosansky13Using} (denoted $\doab$).
...
First, the algorithm needs to calculate the value for each of the successors of the restricted game, for which the current value is not known (lines~\ref{alg:doab:restr1}-\ref{alg:doab:saving}). This evaluation is same as in the case of $\biab$. %backward induction with alpha-beta algorithm.
...
This calculation corresponds to a situation where player $i$ achieves the best possible utility by playing action $a_i$ against all other actions from the strategy of the opponent and it needs to achieve at least $\lambda_{a_i}$ against $a_{-i}$ so that expected value for playing $a_i$ is at least $v_i^{BR}$.}
...
When extracting strategies from $\doab$, we proceed exactly as in the case of $\biab$: either a double-oracle is initialized and solved for a certain matrix game and we keep equilibrium strategies from the final restricted game, or the strategy is extracted from the serialized alpha-beta algorithms as before.
...
Moreover, the worst case for the double-oracle algorithm occurs when all actions in this state must be added and only a single action is added in each iteration causing the largest number of iterations repeatedly resolving the linear program.
...
As we show in Subsection~\ref{sec:exp:brps}, the specific tie-breaking rule used here can lead to a significant effect on the quality
...
Another common choice is to use the Exponential-weight algorithm for Exploration and Exploitation (Exp3)~\cite{Auer2003Exp3} independently for each of the players.
...
\reviewchange{Let $\sigma^t_i$ be the strategy used at time $t$.} After $T$ iterations in the particular node, the average strategy is
\begin{equation}
...
Each node stores for each player: $R_i(s,a)$ the cumulative regret $R_i^T(I,a)$ for information set $I$ of player $i$ in state $s$ and action $a$,
