...
\Output{$(x,q,u_i)$: $x$ -- tail probability ($\pi(h,z)$); $q$ -- sample probability ($q(z)$); $u_i$ -- utility of the leaf}
  ...
  \Else{
    ...
    Sample action $a_{-i}$ from $\sigma_{-i}$\;
    $(x,q,u_i) \gets$ OOS-Rollout$(\cT(s,a_i,a_{-i}), p \cdot \tfrac{1}{|\cA_i(h)|}\tfrac{1}{|\cA_{-i}(h)|})$\label{alg:oos:rollout}\;
  }
