...
\Repeat{$\alpha_s = \beta_s$}{
	...
	$\left\langle v^{BR}_i, a^{BR}_{i} \right\rangle \leftarrow $ BR$(s, i, \delta'_{-i}, \beta_s)$\;\label{alg:doab:br1}
	$\left\langle v^{BR}_{-i}, a^{BR}_{-i} \right\rangle \leftarrow $ BR$(s, -i, \delta'_{i}, -\alpha_s)$\; \label{alg:doab:br2}
...
\Output{$(x_i,q_i,u_i)$: $x_i$ -- $i$'s contribution to tail probability ($\pi^{\sigma}(h,z)$); $q_i$ -- $i$'s contribution to sample probability ($q(z)$); $u_i$ -- utility of the sampled leaf}
  ...
  \ElsIf{$s \in \cC$ is a chance node}{
    Sample $s'$ from $\Delta_c(s)$ \;         \label{alg:oos:chance1}
