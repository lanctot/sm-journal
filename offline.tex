...
Solving the computationally expensive linear programs in backward induction is necessary in game states that require mixed strategies. However, many realistic games include subgames that can be solved using only pure strategies. These subgames can be found efficiently by transforming the simultaneous move game into a perfect information extensive-form 
...
The computational complexity of solving a matrix game by linear programming can be reduced by their incremental construction using the iterative double-oracle algorithm~\cite{McMahan03Planning}. The following algorithm incorporates this idea to $\biab$, which leads to additional pruning of the search tree.
...
Another common choice of a selection function is to use the Exponential-weight algorithm for Exploration and Exploitation (Exp3)~\cite{Auer2003Exp3} independently for each of the players. Unlike with UCT, two players using Exp3 in a single stage matrix game are guaranteed to converge to a Nash equilibrium; hence, we can expect a good performance of this selection function even in multi-stage games.
...
The last selection function we propose is inspired by regret matching \cite{Hart00}, which forms the bases of the successful algorithms for solving imperfect information games \cite{CFR}.
