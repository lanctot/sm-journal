...
We can now generalize this lemma to games trees with multiple simultaneous moves.
...
The value of the serialized game is an upper bound on the value of the simultaneous moves subgame for player $i$.
\end{lemma}
\begin{proof}
...
By induction we assume the algorithm computes for state $s$ some $A'$ so that each value in matrix $A'$ is greater or equal than $A$: 
