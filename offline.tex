...
Once the algorithm computes the value of each possible subgame following the current state $s$, the matrix game $A$ is complete and the algorithm
solves the matrix game $A$ using standard linear program (LP) for solving normal-form games:
\begin{eqnarray}
\max & v_s & \\
...
\end{eqnarray}
Using the linear programming, the algorithm computes the value of the matrix game $v_s$, which is propagated to the predecessor~(line~\ref{alg:bi:stop2}).
If the algorithm evaluates a terminal state, it directly returns the utility value of the state~(line~\ref{alg:bi:stop1}).
A strategy can be easily extracted from the algorithm by keeping the equilibrium strategies $\sigma_i(s,a)$ from LP for matrix games $A$ in each state.
\reviewchange{To be clear, solving a game by solving this linear program yields both the optimal value and the strategy that achieves it.}
...
Furthermore, we can generalize this argument using induction to game trees with multiple simultaneous moves -- if we obtain an upper bound from the perspective of player $i$ by solving each subgame with $d$ steps, then serializing the additional step of a $(d+1)$-step simultaneous move game can again yield value only greater or equal than the value of the original simultaneous move game~\cite{Bosansky13Using}.
}
\begin{figure}
\centering
%\begin{tabular}{|l|c|c|}
...
As before, the algorithm uses the value directly (line~\ref{alg:biab:saving}) instead of performing a recursive call~(line~\ref{alg:biab:recursive}) if the bounds are equal.
...
\If{$\left(s\;\textrm{is root}\right) \wedge \left(\textrm{alpha-beta}(s,i) = \textrm{alpha-beta}(s,-i)\right)$} {\Return $\textrm{alpha-beta}(s,-i)$} \label{alg:biab:stop15}
\For{$r \in \cA_1(s)$}{
	\For{$c \in \cA_2(s)$} {
		$A_{rc} \leftarrow 0$\;
...
In the best case (i.e., there exists a pure NE), the algorithm finds the solution by solving each serialization exactly once starting from the root state.
...
%  \Return $\beta_s$
%}
\If{$\left(s\;\textrm{is root}\right) \wedge \left(\textrm{alpha-beta}(s,i) = \textrm{alpha-beta}(s,-i)\right)$} {\Return $\textrm{alpha-beta}(s,-i)$} \label{alg:doab:stop15}
...
Throughout the algorithm we use, as before, $v^i_{s'}$ to denote the upper bound of the value of the subgame rooted in state $s'$ calculated using alpha-beta$(s',i)$. These values are calculated on demand, i.e., they are calculated once needed and cached until the game for state $s$ is solved.
...
Before evaluating the successors, the algorithm determines, whether current action of the searching player $a_i$ can still be the best response action against the strategy of the opponent.
\reviewchange{In order to determine this, the algorithm computes value $\lambda_{a_i}$ that represents the lowest possible expected utility this action must gain against the current action of the opponent $a_{-i}$ in order for action $a_i$ to be the best response.
...
If value $\lambda_{a_i}$ is strictly higher than the upper bound of the successor (i.e., the value $v^i_{\cT(s,a_i,a_{-i})}$) then the algorithm knows that the action $a_i$ can never be the best response action, and can proceed with the next action (line~\ref{alg:br:next}).
\reviewchange{$\lambda_{a_i}$ is recalculated for each action of the opponent since the upper bound values of the successor nodes $s'$ can become tighter due to the recursive call (line~\ref{alg:br:recursive}).}
%
...
Finally, we describe algorithms based on Counterfactual Regret (CFR; a notion of regret at the information set level), first designed for extensive-form games with imperfect information~\cite{CFR} and first applied to simultaneous move games in~\cite{Lanctot13Goofspiel}.
...
OOS appears similar to SM-MCTS using the RM selection mechanism (Section~\ref{sec:rm}). However, there are a number of differences:
