...
We now turn present a thorough experimental evaluation of the described algorithms.
...
\reviewchange{The only exception was Goofspiel with chance, where both Exp3 and RM converge faster with the parameter set to $0.3$.}
...
Finally, we turn to the comparison of the algorithms in the online setting and we present results from head-to-head tournaments in each game.
...
Each of the described algorithms was implemented in a generic framework for modeling and solving extensive-form games\footnote{Source code is available at the web page of the authors. %\texttt{http://agents.felk.cvut.cz/topics/Computational\_\newline game\_theory}. 
...
where $d$ is a parameter of the game. %\vlisy{did we not use 1-based game at the end?} %[bb] no
...
We refer to this setting as the \emph{fixed sequence} of cards.
Alternatively, we can treat chance in the standard way, where chance nodes determine the card that gets drawn.
We refer to this setting as the \emph{stochastic sequence}.
...
The formal definition follows ($c_i$ is the sum of values of the remaining cards of player $i$):
...
\reviewchange{$b = 0$ otherwise.}
Again, we use $\tanh$ to scale the value into the interval $[-1,1]$ \reviewchange{only in the win-tie-lose case}.
...
In summary, with this randomization UCT appears to be converging to an approximate equilibrium in this game but not to an
...
On the other hand, the success of serialized alpha-beta algorithms is significantly lower and it takes both algorithms much more time to solve the games of the same size.
With $7$ cards, $\biab$ evaluates more than $2\times10^6$ nodes and it takes the algorithm on average $32$ minutes to find the solution.
...
\begin{figure}[t!]
	...
	\end{subfigure}
\caption{Convergence of the sampling algorithms on Goofspiel with $4$ cards and chance nodes.
The vertical lines correspond to the computation times for the exact algorithms.
(Top) Goofspiel with win-tie-lose utility values;
...
This leads to additional reductions in the computation time for $\doab$ compared to $\biab$ and all the other algorithms.%\vlisy{This argument is not very consistent with $\biab$ evaluating $\tfrac{1}{1000}$ of the nodes BI evaluates, while DO evaluating $\tfrac{1}{20}$. The reason is likely the cost of serialized searches.}
...
We now turn to the convergence of the sampling algorithms.
...
Many instances of the Oshi-Zumo game have Nash equilibria in pure strategies regardless of the type of the utility function.
...
\reviewchange{
Figure~\ref{fig:off:res:oz} depicts the results with the parameter $K$ set to $4$ and for two different settings of the utility function\footnote{\reviewchange{We have also performed the same experiments with $K$ set to $3$, but the conclusions were the same as in case $K=4$.}}; either win-tie-lose utilities (left subfigure) or point difference utilities (right subfigure).}
...
\textsc{DO} improves the performance (the algorithm evaluates $2.8\times10^6$ nodes in $17$ minutes \reviewchange{for win-tie-lose utilities; the performance is slightly worse for point utilities: $5\times10^6$ nodes in $23$ minutes).}
...
%\bbosansky{the? ... this is a general question and maybe we should have `the' in each caption of the figures}
% This is ok. And definitely not in the captions
...
\reviewchange{Moreover, the convergence takes even more time in the point utility case, since the range of the utility values is larger.
OOS is again the fastest and converges to error $0.45$ within the time limit, RM to $1.41$, UCT ($C=4$) to $3.1$, and Exp3 to $3.7$.}
...
In the first variant of the randomly generated games we used games with utility values randomly drawn from a uniform distribution on $[0,1]$.
...
Figure~\ref{fig:off:conv:rg} depicts the results for convergence of the sampling algorithms for a random game with correlated utility values, branching factor set to $4$ and depth $5$.
...
Performance of the exact algorithms in Tron is affected by the fact that pure NE exist in all smaller instances \reviewchange{
...
Finally, we also include a random player (denoted RAND) into the tournament to confirm that the algorithms choose much better strategies than the simple random game play.
...
We report expected rewards and win rates of the algorithms, in which a tie counts as half of a win.
...
\reviewchange{In this subsection, we show cross tables of each algorithm (in each row) matched up against each competitor algorithm (in each column).}
Each entry represents a mean of at least $1000$ matches with the half of the width of the $95\%$ confidence interval show in parentheses, \eg $52.9(0.3)$ refers to $52.9\% \pm 0.3\%$.
...
The performance of the other sampling algorithms is very similar against $\doab$, with Exp3 winning the least in the reward optimization.
...
The size of the game is large with strictly more than $10^{15}$ terminal states 
%(the number of choices of a single player in the game; hence, the square of this number is the upper bound on the number of leaves)
and 50 actions for each player in the root.

\reviewchange{The results are depicted in Table~\ref{fig:matches:oz}. As in the case of Goofspiel, we show the results for both the win rate as well as the point utilities. Moreover, our evaluation function in Oshi-Zumo is much more accurate than the one in Goofspiel and $\doab$ is clearly outperforming all sampling algorithms when they do not use any domain specific knowledge. Therefore we also run experiments where the sampling algorithms also use an evaluation function instead of random rollout simulations.}
...
In the offline experiment (Figure~\ref{fig:off:conv:oz}), none of the sampling algorithms were able to converge anywhere close to the equilibrium in a short time. Moreover, the game used in the offline experiments was orders of magnitude smaller (there were only $10$ coins for each player).
...
We now turn to the experiments with the evaluation function, the results of which are presented in the third table of Table~\ref{fig:matches:oz}.
...
The large variant of Tron in our evaluation was played on an empty $13\times 13$ board. The branching factor of this game is up to 4 for each player and its depth is up to 83 moves. This variant of Tron has more than $10^{21}$ terminal states\footnote{The number only estimates the number of possible paths when both players stay on their half of the board.}.
...
\reviewchange{UCT is the strongest opponent for $\doab$ -- UCT loses 53.8\% of matches and wins over all other sampling algorithms in mutual matches. 
...
In this game, the variance of the regret updates is likely not the key factor, since it is between 20 and 40.
However, only $1 \times 10^3$ out of $12 \times 10^3$ iterations per second update regrets in the root.}

\reviewchange{The good performance of $\doab$ is consistent with the previous analysis in Tron where the best-performing algorithms, including the winner of the 2011 Google AI Challenge, were based on depth-limited minimax searches~\cite{DenTeuling12Tron,Perick12Comparison}.}
...
Finally, we compare the algorithms on the pursuit-evasion game on an empty $10\times 10$ grid with $15$ moves time limit and 10 different randomly selected initial positions of the units. The branching factor is at most 12, causing the number of terminal states to be less than $10^{16}$.
...
The clear winner is UCT that generally captures the evaders in approximately 40\% of the matches.
The second best pursuer is $\doab$ and the weakest is OOS that captures the non-random opponents in less than 10\% of the cases.
...
UCT was the best against OOS and RM, but $\doab$ was the best against UCT and Exp3.
...
\reviewchange{
The exploration parameters can have a significant influence on the performance of the algorithm.
...
In UCT, less exploration is generally better, but the sudden drop of performance against Exp3 causes the optimum to be at 0.6.
In Exp3, the optimal exploration parameter against $\doab$ would be even greater than 0.5, while the optimum against OOS would be 0.2.
These kinds of inconsistencies are common with the Exp3 algorithm. In the mean over all opponents, the optimum is 0.3.
...
%\bbosansky{Summary of \textbf{the?} Online Search Experiments}
...
\reviewchange{
Several conclusions can be made from the head-to-head comparisons of the algorithms in larger games.
...
Second, $\doab$ with a good evaluation function often wins over the sampling algorithms without a domain specific knowledge.
This is not the case with a weaker evaluation function, as we can see in Goofspiel.
Moreover, when the sampling algorithms are allowed to use the evaluation functions, $\doab$ is outperformed by UCT in both domains tested with evaluation function and also by RM in Tron.
...
Given the similar nature of OOS and RM, one might wonder why RM typically performs better than OOS in online search, despite OOS being the only algorithm with provable convergence properties and the fastest converging algorithm in the offline setting. 
In this subsection, we investigate this phenomenon and present the results of additional experiments.

We need to look at the convergence properties of OOS, which is essentially an application of outcome sampling MCCFR.
From the convergence bound of outcome sampling MCCFR presented in \cite{Gibson12probing}, after $T$ iterations the strategy produced by the algorithm is an $\epsilon$-Nash equilibrium with probability $1-p$ and
...
The only difference is that they are not counterfactual, \ie they take into account only the value of the current sample and not the expected value of the strategy used throughout the entire game. We show these variances for
