...
We compute the mean over at least $30$ runs for each game setting.
...
Goofspiel is a card game that appear as a common example of a simultaneous move game (\eg \cite{Ross71Goofspiel,Rhoads12Computer,Saffidine12SMAB,Lanctot13Goofspiel}).
There are $3$ identical decks of $d$ cards with values $\{0,\dots, (d-1)\}$, one for chance and one for each player, 
where $d$ is a parameter of the game. Standard Goofspiel is played with $13$ cards.
...
where the sequence of the cards are randomly chosen at the beginning of the game and it is known to both players.
...
The number of iterations per second is similar to in Goofspiel, with Exp3 being the exception able to achieve more than $6.5\times10^4$ iterations per second, which is still significantly the lowest number of iterations.
...
Each entry represents a mean over at least $1000$ matches with the half of the width of the $95\%$ confidence interval in brackets, \eg $52.9(0.3)$ refers to $52.9\% \pm 0.3\%$.
