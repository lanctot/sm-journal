...
Since all units move simultaneously, the game has larger branching factor (up to $16$ for the pursuer).
...
enabled in all of our experiments. %mlanctot: is this true? %bbosansky: yes, we have run again experiments with randomized uct where deterministic uct had been used before
...
We first compare the offline performance of the algorithm. 
...
While \textsc{BI} algorithm evaluates on average more than $3.2\times10^7$ nodes in the setting with $7$ cards in more than $3$ hours, $\biab$ evaluates only $198986$ nodes in less than $5$ minutes. 
...
The results on Goofspiel represent a significant improvement over the Goofspiel results of the pruning algorithm 
...
While Exp3 eventually converges close to $0$, the exploitability of UCT decreases rather slowly and it was still over $0.35$ at the time limit of $300$ seconds. The best C constant for UCT was 5 in this setting. While setting lower constant typically improved slightly the convergence during the first iterations, the final error was always larger.
...
Large parts of pursuit-evasion games can be solved by serialized alpha-beta algorithms.
...
Figure~\ref{fig:off:conv:rg} depicts the results for convergence of the sampling algorithms for the random game with correlated utility values, branching factor set to $4$ and depth $5$. The number of iterations per second is very similar to Goofspiel, with Exp3 being the exception able to achieve more than $6.5\times10^4$ iterations per second, which is still significantly the lowest number of iterations.
...
Each result represents a mean of at least 1000 matches with the half of the width of the 95\% confidence interval in brackets, \eg 49(0.3) refers to 49.3\% $\pm$ 0.3\%. 
...
This is noticeable thanks to the result of RAND player, that performs reasonably well (RAND typically loses almost every match in all the remaining game domains). 
...
These estimates may be misleading when using the evaluation function in Oshi-Zumo, since the evaluation function is not very stable and can change very dramatically after a single move. 
...
\begin{tabular}{|r|cccccc|}\hline
5s&$\doab$&OOS&UCT&EXP3&RM&RAND\\\hline
$\doab$&\textbf{49.6(2.9)}&\textbf{60.2(2.8)}&\textbf{49.8(2.9)}&\textbf{49.6(2.9)}&\textbf{49.1(2.8)}&\textbf{87.6(1.9)}\\
...
%OOS loses also against all MCTS algorithms, which perform practically the same among each other. These performance results are not supported by the speed of convergence in , where Exp3 converges consistently slower than OOS on the small variant of the game.
...
The influence of the exploration parameter in OOS is not statistically significant after 1000 matches played for this table. The differences with 5 seconds per move were even smaller. While the default parameter is likely not optimal in this setting, it is still meaningful.
...
However, when altering the parameters within reasonable boundaries, the differences are not that significant in affecting the final conclusion of the experiments.
...
Finally, RM seems to be consistently the best performing algorithm in game playing for this class of games. Even with similar number of iterations, it builds a substantially deeper tree (see Figure~\ref{fig:tab:depthsIters}), which indicates the algorithm is able to better target the search to important parts of the tree. Out of all the games and both time settings, it looses only in Oshi-Zumo with evaluation function, where it is outperformed by UCT.
