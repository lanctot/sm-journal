...
\reviewchange{The only exception was Goofspiel with chance, where both Exp3 and RM converge more quickly with the parameter set to $0.3$.}
...
We used larger instances of \bbosansky{the?}games and compare the algorithms based on the actual game play with a limited time for each move. 
...
This causes additional improvement in the computation time for $\doab$ compared to $\biab$ and all the other algorithms.%\vlisy{This argument is not very consistent with $\biab$ evaluating $\tfrac{1}{1000}$ of the nodes BI evaluates, while DO evaluating $\tfrac{1}{20}$. The reason is likely the cost of serialized searches.}
...
Figure~\ref{fig:off:conv:oz} depicts the results for \bbosansky{the? ... this is a general question and maybe we should have `the' in each caption of the figures}convergence of the sampling algorithms for the game with $10$ coins, $K$ set to $3$ and the minimum bid set to $1$. This is an easy game for $\doab$ and $\biab$ with a pure NE and both of these algorithms are able to solve the game in less than a second ($0.73$). However, due to a large branching factor for both players ($10$ actions at the root node for each player) all sampling algorithms converge extremely slowly. The performance of the algorithms in terms of iterations per second is similar to the previous games, however, OOS is slightly better in this case with $1.9\times10^5$ iterations per second compared to the RM with $1.6\times10^5$ iterations per second.
...
\reviewchange{In this subsection, we show cross tables of each algorithm (on\bbosansky{in?} the row) matched up against each competitor algorithm (on\bbosansky{in?} the column).}
Each entry represents a mean of at least $1,000$ matches with the half of the width of the $95\%$ confidence interval in \bbosansky{the?}brackets, \eg $52.9(0.3)$ refers to $52.9\% \pm 0.3\%$.
...
\reviewchange{In \bbosansky{the?}head-to-head comparison, our focus is primarily on the standard Goofspiel with 13 cards and chance nodes. Additionally, for the sake of consistency with the offline results, we also evaluate the variant with a fixed known sequence of cards. The full game has more than $2.4\times 10^{29}$ terminal states and the variant with a fixed sequence has still more than $3.8\times 10^{19}$ terminal states. The results are presented in Table~\ref{fig:matches:goof}, where the top table shows the win rates of the algorithms in the full game and the other two tables the win rates and \bbosansky{the?}expected number of points gained by the algorithms in the game with a fixed point card sequence. The results for the fixed card sequence are means over $10$ fixed random sequences. For each table, the algorithms were set up to optimize the presented measure (\ie win rate or points) and the exploration parameters were tuned to the values presented in the header of the table.

First, we can see that finding a good strategy in Goofspiel is difficult for all the algorithms.
This is noticeable thanks to the results of the RAND player, that performs reasonably well
...
\reviewchange{The situation remains similar when the algorithms optimize \bbosansky{the?}point utilities.}
...
In UCT, a smaller exploration is generally better, but the sudden drop of performance against Exp3 causes the optimum to be at 0.6.
In Exp3, the optimal exploration parameter against $\doab$ would be even more than 0.5, while the optimum against OOS would be 0.2.
...
Fourth, when the algorithms have five times more time for finding a move to play, the differences between win-rates of the sampling algorithms are getting smaller. \bbosansky{A?}Longer thinking time has also the same effect in the parameter tuning and it also significantly improves the performance of the sampling algorithms against backward induction. This is expected, since the difference is often too small for the $\doab$ algorithm to reach a greater depth, while it is sufficient for the sampling algorithms to execute five times more iterations improving their strategy.
