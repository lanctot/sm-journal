...
OOS has the weakest performance in spite of good convergence results in the offline settings (see~Figure~\ref{fig:off:conv:rg}). The reason is the quickly growing variance and decreasing number of samples in longer games, which we discuss in more details in Section~\ref{???}. OOS performs 20 thousand iterations per second and only around 3 thousand of them actually update the regrets in the root. Even though, the variance of the updates ranges between $1.5\times 10^5$ and $2.5\times 10^5$. It is approximately 10 times more than for the Goofspiel with similar number of root updates.
...
OOS&0.1&51.0(3.2)&47.4(3.1)&53.4(4.3)&48.6(3.0)&43.9(3.0)&48.86\\\hline
...
UCT&0.4&39.0(3.0)&54.9(3.0)&61.6(3.1)&58.6(3.8)&49.5(4.8)&52.72\\\hline
...
EXP3&0.1&42.3(3.4)&44.9(3.0)&48.9(3.0)&51.2(3.0)&40.9(3.0)&45.64\\\hline
...
First, the fast convergence and low exploitability of OOS in the smaller variants of the games is not a very good predictor of its performance in the online setting. Even though OOS is the clearly the fastest converging algorithm in the smaller games, it rarely performs the best in the larger games.

One possible reason could be that with higher exploration parameter required for faster convergence, the part of the game tree constructed by OOS is too wide and shallow, and does not allow it to find important situations later in the game. However, Table~\ref{fig:tab:depthsIters} shows that while OOS builds a substantially less deep tree as RM, it is often deeper than thefor other MCTS algorithms which performed better. If we set the exploration parameter to smaller numbers, and also generally deeper in the tree, the importance sampling correction of regret updates causes very high variance of their values, which generally slows-down the convergence to good solutions and may result in building the tree in less useful directions.\vlisy{add the variance graphs}

Second, while $\doab$ with a good evaluation function wins over the sampling algorithms without domain specific knowledge, it is not necessary the case with a weaker evaluation function, as we can see in Goofspiel. Moreover, when the sampling algorithms are allowed to use the evaluation functions, $\doab$ is in all our experiments always outperformed by UCT, and often even by RM and Exp3. 

Third, when the algorithms have more time for finding the move to play, the differences between win-rates of the sampling algorithms are getting smaller. Longer thinking time has also the same effect in the parameter tuning and it also significantly improves the performance of the sampling algorithms against backward induction. This is expected, since the difference is too small for the $\doab$ algorithm to reach greater depth, while it is sufficient for the sampling algorithms to execute substantially more iterations that improves their strategy.

Fourth, Exp3 in the presented numerically stable form is not very suitable for games with larger branching factors. The quadratic dependence of the amount of computation on the number of actions and the cost of computing the exponentials causes an order of magnitude smaller number of iterations in Oshi-Zumo, compared to all other sampling algorithms. This caused bad performance mainly in the optimizing win rate in this game.

Finally, UCT seems to be overall the best performing algorithm in game playing for this class of games. Even though it is not guaranteed to converge to the optimal solution, it quickly finds good strategies, which are also able to exploit weaker opponents. It looses only to $\doab$ when only the latter is allowed to use the evaluation function and to RM and OOS in Goofspiel and Tron with evaluation function. 

\vlisy{advantage of UCT is that it is the only algorithm that adapts the amount of exploration to the difference in expected utility}

%Even with similar number of iterations, it builds a substantially deeper tree (see Table~\ref{fig:tab:depthsIters}), which indicates the algorithm is able to better target the search to important parts of the tree. Out of all the games and both time settings, it loses only in Oshi-Zumo with evaluation function, where it is outperformed by UCT.
