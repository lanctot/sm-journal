...
We measure the overall computation time for each of the algorithms and number of evaluated nodes -- i.e., nodes for which the main method of teh backward induction algorithm executed (i.e., nodes evaluated by serialized alpha-beta algorithms are not included in this count, since they may be evaluated repeatedly).
...
RM is the fastest with more than $2.6\cdot 10^5$ iterations per second, OOS has around $2\cdot10^5$ iterations, UCT $1.9\cdot10^5$, and Exp3 only $5.4\cdot10^4$ iterations.
...
These parts typically corresponds to clearly winning, or clearly loosing positions for a player; hence, serialized alpha-beta algorithms are able to prune substantial portion of the space. 
...
Therefore, serialized alpha-beta is not able to solve all sub-games, while double-oracle provides additional pruning since many of the actions in the sub-games are leading to the same outcome and not all of them are required to find equilibrium strategies.
...
UCT achieved slightly less ($1.7\cdot10^5$ iterations per second), and Exp3 only $2.6\cdot10^4$ iterations.
...
Figure~\ref{fig:off:conv:rg} depicts the results for convergence of the sampling algorithms for the random game with correlated utility values, branching factor set to $4$ and depth $5$. The number of iterations per second is very similar to Goofspiel, with Exp3 being the exception able to achieve more than $6.5\cdot10^4$ iterations per second, which is still significantly the lowest number of iterations.
...
OOS is the fastest performing $1.3\cdot 10^5$ iterations per second, RM achieves $1.2\cdot 10^5$ iterations, UCT only $8\cdot10^4$, and Exp3 is again the slowest with $7.8\cdot10^4$ iterations per second.
...
The performance of $\doab$ decreases with more time and with 5 seconds, it already wins only over OOS (in 60.5\% matches), and playing tie with the other sampling algorithms. 
...
As before, OOS is the weakest of the sampling algorithms. In the setting with 5 seconds it won only 15.1\% matches against RM. 
Among MCTS algorithms, UCT performs the worst and looses 61.6\% matches against RM.
...
For 5 seconds setting, RM without any domain knowledge already performs significantly better (wins 90\% as an evader, while $\doab$ wins only 70.4\% of games as an evader).
Comparing against the other opponents, RM has statistically significant difference of performance only against Exp3, against which $\doab$ on the first position wins 73\% of matches and RM wins 78.5\% of matches. The other two MCTS variants have similar performance in both time settings and OOS is consistently the weakest. 
...
First, the fast convergence and low exploitability of OOS algorithm in the smaller variants of the games is not very good predictor of its performance in the online setting. OOS was consistently one of the worst performing algorithm. \bbosansky{Why?}
