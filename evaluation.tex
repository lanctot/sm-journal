...
We can use an assumption made in the previous work that used Goofspiel as a benchmark for evaluation of the exact offline algorithms \cite{Saffidine12SMAB}, where the sequence of the cards of nature is randomly chosen at the beginning of the game and it is known to both players.
We refer to this setting as with the \emph{fixed sequence} of cards of nature.
Alternatively, we can treat nature in a standard way, where the players do not know the exact future sequence of the cards of nature (we refer to this setting as with an \emph{unknown sequence}).
...
Again, we use $\tanh$ to scale the evaluation function into the interval $[-1,1]$ \reviewchange{for the win-tie-loose case}.
...
\centering
	...
	\begin{subfigure}{0.49\textwidth}
 	...
 	\end{subfigure}
\caption{Running times of exact algorithms on Goofspiel with fixed sequence of the nature's cards for increasing size of the deck; subfigure (a) depicts the results with win-tie-loose utilities, (b) depicts the results with point utilities.} \label{fig:off:res:gs}
\end{figure}

We now describe the results for the card game Goofspiel.
First, we analyze the games with a fixed sequence of the cards by nature.

\paragraph{Exact algorithms with fixed sequence}
...
\reviewchange{
\paragraph{Exact algorithms with unknown sequence}
...
For $7$ cards, the game tree has more than $10^{11}$ nodes, which is $4$ orders of magnitude more than in the case with fixed sequence of nature's cards.
...
\paragraph{Sampling algorithms with fixed sequence}
...
\begin{figure}[t!]
	...
	\end{subfigure}
\caption{Convergence of sampling algorithms on Goofspiel with $5$ cards and fixed sequence of nature's cards.
...
Figure~\ref{fig:off:conv:gs} depicts the results for Goofspiel game with $5$ cards with fixed sequence cards of nature (note the logarithmic horizontal scale).
...
\reviewchange{
\paragraph{Sampling algorithms with unknown sequence}
...
Goofspiel: 13 cards, unknown sequence of cards of nature, win rate
...
OOS&73.3(2.7)&$\bullet$&\textbf{51.2(2.1)}&52.5(2.2)&47.5(3.0)&81.4(1.7)\\
...
Goofspiel: 13 cards, known sequence of cards of nature, win rate
\begin{tabular}{|r|cccccc|}\hline
...
Goofspiel: 13 cards, known sequence of cards of nature, point utilities
\begin{tabular}{|r|cccccc|}\hline
&$\doab$&OOS(0.3)&UCT(0.8)&EXP3(0.2)&RM(0.1)&RAND\\\hline
...
In the game with unknown point card sequence, OOS, UCT and RM make approximately $10 \times 10^3$ iterations in the 1 second time limit in the root of the game. Exp3 is slightly slower with $8 \times 10^3$ iterations.
...
The sampling algorithms in the second game variant perform the same number of samples as in the first variant, with the exception of UCT, which performs $12 \times 10^3$ iterations per second.
...
In summary, RM is the only algorithm that did not loose significantly against any other sampling algorithm in any other game variant and often wins significantly.
...
\reviewchange{The reason is the quickly growing variance and decreasing number of samples in longer games, which we discuss in more details in Section~\ref{sec:oos_vs_rm}. OOS performs $20 \times 10^3$ iterations per second and only around $3 \times 10^3$ of them actually update the regrets in the root. \bbosansky{We need one sentence here about the reason for this difference (even if it seems obvious to you).}
Even though, the variance of the updates ranges between $1.5\times 10^5$ and $4\times 10^5$. 
It is approximately 10 times more than for the Goofspiel with similar number of root updates.
\bbosansky{These last two sentences are not clear. I do not understand them.}}
...
This is again because of the low need for mixed strategies in this game (see Section~\ref{sec:eval:domains}).
...
Interestingly, while UCT performs very well against $\doab$ and wins 57.3\% of matches, it is not winning against any other sampling algorithm. 
...
\reviewchange{Let's first look at the differences in the performance of the algorithms on the side of the pursuer, which are more consistent. We need to compare the different columns, in which the pursuer tries to minimize the values.
...
\bbosansky{I am unable to follow the text below ... I guess it would be beneficial to avoid most of the math if possible and if something is necessary to explain then repeat the meaning because people have forgotten everything while reading through these long experiments.}
...
Since outcome sampling uses unbiased estimators of its counterfactual values,
from~\cite{Gibson12probing} the bound on its overall average regret after $T$ iterations is, with probability $1-p$, 
\[ \bar{R}_i^T \le O\left( |\cS| \frac{\tilde{\Delta}_i }{\sqrt{T}} + \sqrt{\frac{\bVar}{pT} + \frac{\bCov}{p}} \right), \] 
where $\tilde{\Delta}_i$ is a constant determined by the game's structure, and 
$\bVar$ and $\bCov$ are the maximal variance and covariance of the terms $r^t(s,a) - \tilde{r}^t(s,a)$ over all information sets of
player $i$, actions, and time steps. Since $\bar{R}^T_1,\bar{R}^T_2 \le \epsilon \Rightarrow (\bar{\sigma}^T_1, \bar{\sigma}^T_2)$ is a 
$2\epsilon$-NE, then the variance of estimates in OOS affects it rate of convergence.

Computing these quantities exactly is prohibitively expensive, and since $r^t(s,a) \in [0,1]$ the scale is much smaller than $\tilde{r}^t(s,a)$, 
we can get a good estimate by instead computing
$\widehat{\bVar} = \text{Var}[\max_{a \in \cA(s)} \tilde{r}^t(s, a)]$. We do this for a representative state $s$, \ie the root. Regret matching
also computes an $\tilde{r}^t(s,a)$ quantity at each node, the only difference is that they are not counterfactual. We show these variances for
...
\hline
RG(15,5)	& $1$	& $20,730$	& $198,220.92$	& $22,100$	& $3,306.38$ \\
RG(15,5)	& $2$	& $16,420$	& $157,326.58$	& $17,357$	& $3,480.39$ \\
RG(15,5)	& $3$	& $16,222$	& $400,370.47$	& $7,897$	& $552.24$ \\
\hline
...
to avoid the uncertainty introduced by the variance. This problem does not show up in the smaller games, because the the updates have smaller variance and OOS performs much more samples per second. For example, in Goofspiel(5) used for offline convergence experiments, OOS performs approximately $1.5 \times 10^5$ iterations per second and the variance is only around 350. 
