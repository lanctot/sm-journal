...
The algorithms include both backward induction methods with very efficient pruning, as well as Monte-Carlo sampling algorithms.
...
Thorough experimental evaluation in six substantially different games shows that the proposed algorithms can compute the optimal strategy in less then 2\% of the time required by the existing algorithms. Furthermore, the novel sampling algorithm dominates in the head-to-head tournament with pre-existing methods.
...
algorithm on a suite of games. Our collection of games covers many games used for benchmark purposes
