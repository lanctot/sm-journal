...
%\footnotetext[3]{This Google DeepMind, London, United Kingdom.}
...
At each stage, a player does not know what action the other player will take, but otherwise knows the full state of the game.
\reviewchange{
...
These situations occur in many real-world scenarios like auctions~(\eg~\cite{Keuter1997}) or autonomous driving, and many video
...
action while they play it. %Chosen actions are revealed publically by each player after they have been played.
...
Section~\ref{sec:relwork} follows with a list and a discussion of the existing algorithms in the related work. In
...
This paper presents detailed descriptions of the recent state-of-the-art exact~\cite{Bosansky13Using} and approximative
...
\begin{itemize}
\item We present the latest variants of the state-of-the-art algorithms under a single unified framework and combine the offline and online computation perspectives that have been previously analyzed separately.
\item We describe the first adaptation of backward induction and the double-oracle algorithm with serialized bounds~($\doab$)~\cite{Bosansky13Using} to the online search setting in simultaneous move games using heuristic evaluation functions.
\item We describe a tailored version of Online Outcome Sampling~(OOS)~\cite{15aamas-iioos} and its analysis for the first time in two-player simultaneous move games.
\item We provide a wide experimental analysis and a comparison of these and other algorithms on five different specific games and randomly generated games.
...
Our algorithms thus allow computing offline strategies in larger games than before (using $\doab$) and provide a comparable performance in the online game-playing that is less sensitive to chosen parameters (MCTS-RM) or guarantees to closely approximate the optimal strategies given enough time (OOS). Since we describe each algorithm in a domain independent manner, they can be further tailored to a specific domain and additional improvements in the scalability and/or the game-playing performance can be achieved.
}
