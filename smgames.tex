...
\reviewchange{These strategies are often called {\it randomized}, or {\it mixed} because they represent a mixture over {\it pure} strategies, each of
...
$\pi^\sigma(h) = \pi_1(h) \pi_2(h) \pi_\star(h)$, where each $\pi_i(h)$ is a product of probabilities of the actions taken 
...
\reviewchange{For the remainder of the article, we adopt a standard convention that the index $-i$ refers to all other players except player $i$. 
In a game with chance events, then the $-i$ also includes the chance player, so \eg if $i = 1$ then $\pi_{-i}(h) = \pi_2(h) \pi_\star(h)$. 
...
In a matrix game, an action $a_i \in \cA_i$ is dominated if $\forall a_{-i} \in \cA_{-i}, \forall a_i' \in \Sigma_i - \{ a_i \}, u(a_i, a_{-i}) < u(a_i', a_{-i})$. 
\end{definition}
...
A pair of strategies $(\sigma_i, \sigma_{-i})$ is a Nash equilibrium profile if and only if $\sigma_i \in BR_i(\sigma_{-i})$ 
...
A strategy $\sigma_i \in \Sigma_i$ is dominated if $\forall \sigma_{-i} \in \Sigma_{-i} \forall \sigma_i' \in \Sigma_i u(\sigma_i, \sigma_{-i}) < u(\sigma_i', \sigma_{-i})$. 
\end{definition}

No rational player would want to play a dominated strategy, because there is always a better one to play instead, for every strategy the opponent player can use.
}
