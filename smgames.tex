...
joint actions of individual players. We denote $\cA_i(s)$ the actions available to player $i$ in state $s \in \cS$ \reviewchange{and denote the number of actions $|\cA_i(s)|$ as the \emph{branching factor for player $i$}. When the player is not specified, we are talking about branching factor for all players and number of joint actions $|\cA|$.}
