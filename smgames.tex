...
joint actions of individual players. We denote $\cA_i(s)$ the actions available to player $i$ in state $s \in \cS$ \reviewchange{and denote the number of actions $|\cA_i(s)|$ as the \emph{branching factor for player $i$}. When the player is not specified, we mean the joint branching factor $|\cA(s)|$.}
...
\reviewchange{We denote by $\sigma_i(s,a)$ the probability that strategy $\sigma$ assigns to $a$ in $s$.
...
In a matrix game, an action $a_i \in \cA_i$ is strictly dominated if $\forall a_{-i} \in \cA_{-i}, \forall a_i' \in \Sigma_i \setminus \{ a_i \}: u_i(a_i, a_{-i}) < u_i(a_i', a_{-i})$.
\end{definition}

No rational player would want to play a strictly dominated action, because there is always a better action to play instead, for any action used by the opponent. The concept also extends naturally to behavioral strategies.
