...
both actions, \ie play with a {\it mixed strategy}\footnote{\reviewchange{Notice that a pure strategy is also a mixed strategy that assigns probability 1 to a single action and probability 0 to every other action. However, as is standard in this literature, we will use the term ``mixed strategy'' to refer specifically to the case where no action is selected with probability 1.}} (distribution) of $(0.5, 0.5)$ giving an expected payoff of $V^* = 0.5$.
Note that using a mixed (\ie randomized) strategy is necessary in this game.
