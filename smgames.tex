...
Each white matrix corresponds to a state where both players act simultaneously.
The dark squares are terminal states. 
The values shown in the matrices correspond to the values of subgames that can be obtained by the backward induction.\\
...
No rational player would want to play a strictly dominated action, because there is always a better action to play instead regardless which action the opponent is playing. The concept also extends naturally to behavioral strategies.
