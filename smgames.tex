...
\end{subfigure}%\\
\caption{An example of a two-player simultaneous game. % which has Matching Pennies as a subgame. %, and a portion of 3-card Goofspiel including chance nodes (bottom).
...
joint actions of individual players. We denote $\cA_i(s)$ the actions available to player $i$ in state $s \in \cS$ \reviewchange{and denote the number of actions $|\cA_i(s)|$ as the \emph{branching factor for player $i$}. When the player is not specified, we mean the joint branching factor $|\cA|$.}
...
In two-player zero-sum games a (subgame perfect) Nash equilibrium strategy is often considered to be optimal (formal definition follows).
...
In simultaneous move games, at every state $s$ there is a joint action set $\cA_1(s) \times \cA_2(s)$ that either leads to another state $\cT(s,r,c)$ that is either a terminal state or (possibly after a
chance transition) a subgame which is itself another simultaneous move subgame.
...
Given a strategy profile $\sigma = (\sigma_1, \sigma_2)$, define the probability of reaching a history $h$ under $\sigma$ as
...
Finally, define $\Sigma_i$ to be the set of all behavioral strategies for player $i$.
...
optimal behavior for this class of games, to define metrics for evaluation, and within the algorithms as well.

\begin{definition}[Strictly Dominated Action]
In a matrix game, an action $a_i \in \cA_i$ is strictly dominated if $\forall a_{-i} \in \cA_{-i}, \forall a_i' \in \Sigma_i \setminus \{ a_i \}: u(a_i, a_{-i}) < u(a_i', a_{-i})$.
\end{definition}
...
extends naturally to behavioral strategies.
For example, in the game on the right of Figure~\ref{fig:egMatrixGames}, both $b$ and $B$ are strictly dominated.

\begin{definition}[Best Response]
Suppose $\sigma_{-i} \in \Sigma_{-i}$ is a fixed opponent strategy. Define the set of best response strategies
